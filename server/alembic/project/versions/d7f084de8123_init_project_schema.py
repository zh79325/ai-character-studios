...
Revision ID: d7f084de8123
Revises:
Create Date: 2026-08-31 14:13:25.039530

一个项目目录一套。新建或导入项目时对该目录的 `.atelier/project.db` 跑一次，
每个库自己一份 alembic_version，彼此不干扰。

项目库只有这一版：库都是随项目目录新建的，没有需要原地升级的存量数据，所以结构直接
按模型的当前样子建出来，不留一串加列的中间版本。以后要改结构再往后接。
"""
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("render_path", sa.String(length=512), nullable=True),
    ...
    with op.batch_alter_table("conversations", schema=None) as batch_op:
        batch_op.create_index(
            "ix_conversations_target", ["target_kind", "target_ref"], unique=False
        )
