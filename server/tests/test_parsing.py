...
def test_括号里的顿号不当分隔() -> None:
    """选项文字自己就带着括号列举，切进去就把一个选项摆成三个看不懂的碎片，推荐也对不上。"""
    text = (
        "[待选项]\n- 项: 参考作品锚点"
        " / 选项: 《我的世界：地下城》（方块风格、光照、色彩基准）"
        " | 86版西游记卡通形象（角色造型参考）"
        " / 多选: 是 / 推荐: 《我的世界：地下城》（方块风格、光照、色彩基准）\n"
    )

    (group,) = parsing.parse_choices(text)

    assert group.options == (
        "《我的世界：地下城》（方块风格、光照、色彩基准）",
        "86版西游记卡通形象（角色造型参考）",
    )
    assert group.recommended == ("《我的世界：地下城》（方块风格、光照、色彩基准）",)


def test_括号外的顿号照样分隔() -> None:
    """模型也会整行用顿号列选项，那时候还得切开。括号没合上的就当它一直开着。"""
    (both,) = parsing.parse_choices("[待选项]\n- 项: 色温 / 选项: 冷白、暖黄、中性\n")
    assert both.options == ("冷白", "暖黄", "中性")

    assert parsing.parse_choices("[待选项]\n- 项: 色温 / 选项: 冷白（没合上、暖黄\n") == ()
